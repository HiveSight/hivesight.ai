...
CREATE TABLE IF NOT EXISTS public.llm_responses (
    ...
    query_id uuid NOT NULL REFERENCES public.llm_queries(query_id),
...
ON public.llm_responses FOR SELECT
TO authenticated
USING (
  query_id IN (
    SELECT q.query_id FROM public.llm_queries q WHERE q.requester_id = auth.uid()
  )
);

-- Trigger function to handle new llm_query
CREATE OR REPLACE FUNCTION public.handle_new_llm_query()
RETURNS trigger AS $$
DECLARE
...
BEGIN
    ...
    IF NOT public.check_sufficient_credits(NEW.requester_id, required_credits) THEN
        UPDATE public.llm_queries SET status = 'insufficient credits' WHERE query_id = NEW.query_id;
        RETURN NEW;
    END IF;
    ...
    UPDATE public.llm_queries SET status = 'processing' WHERE query_id = NEW.query_id;
    ...
    IF http_status != 200 THEN
        ...
        UPDATE public.llm_queries SET status = 'error' WHERE query_id = NEW.query_id;
        RETURN NEW;
    END IF;
...
CREATE TRIGGER on_llm_query_created
AFTER INSERT ON public.llm_queries
FOR EACH ROW
EXECUTE FUNCTION public.handle_new_llm_query();
